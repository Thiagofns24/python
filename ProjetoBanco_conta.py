...
from utilsBanco.helper import formata_float_str_moeda
...
class Conta:
    ...
    @limite.setter
    def limite(self: object, valor: float) -> None:  #atribuição de um novo valor ao atributo determinado
        self.__limite = valor
    ...
    def depositar(self:object, valor: float) -> None:
        if valor > 0:
            ...
            print('Erro ao efeturar depósito. Tente novamente')
    ...
    def transferir(self: object, destino: object, valor: float) -> None:
        if valor > 0 and self.saldo_toal >= valor:
            if self.saldo >= valor:
                self.saldo = self.saldo - valor
                self.saldo_total = self._calcula_saldo_total
                ...
                destino.saldo = destino.saldo + valor
                destino.saldo_total = destino._calcula_saldo_total
            print('Transferencia realiada com sucesso.')
        else:
